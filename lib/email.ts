...
export async function sendEmail({ to, subject, html, text }: SendArgs): Promise<void> {
  ...
  const from = process.env.EMAIL_FROM ?? "Dezignxo <onboarding@resend.dev>";
...
<html><body style="margin:0;background:#0b0b10;color:#e8e8ed;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <div style="max-width:560px;margin:0 auto;padding:32px 24px;">
    <div style="font-size:20px;font-weight:700;background:linear-gradient(90deg,#a855f7,#7c3aed);-webkit-background-clip:text;background-clip:text;color:transparent;margin-bottom:24px;">Dezignxo</div>
    ...
    <div style="margin-top:32px;padding-top:24px;border-top:1px solid #2a2a35;color:#7d7d8a;font-size:12px;line-height:1.6;">
      You're receiving this because someone (hopefully you) used your email at Dezignxo.
...
export function renderVerifyEmail(name: string, verifyUrl: string): { subject: string; html: string } {
  const subject = "Verify your email — Dezignxo";
  const html = shell(
...
export function renderResetEmail(name: string, resetUrl: string): { subject: string; html: string } {
  const subject = "Reset your password — Dezignxo";
  const html = shell(
          ...
          <tr>
            <td style="padding:18px 44px 32px;">
              <p style="margin:0 0 6px;font-size:12px;line-height:1.6;color:#6b6b78;">
                You're receiving this because you have a Dezignxo account. Every notification also waits for you under the bell icon when you're signed in.
              </p>
              <p style="margin:0;font-size:12px;line-height:1.6;color:#6b6b78;">
                © ${year} Dezignxo — the digital asset marketplace.
              </p>
            </td>
          </tr>
